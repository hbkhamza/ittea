...
function GenerateCheckboxes {
    ...
    foreach ($Item in $Database) {
        ...
        <StackPanel Orientation="Vertical" Margin="0">
                <CheckBox Content="$Content" FontSize="15" Margin="5,8,0,15" Tag="$Tag" $Toggle $Name ToolTip="$CleanedDescription"/>
        </StackPanel>
"@
    }
    return $Checkboxes
...
function Update-Readme {
    ...
    $badgeUrl = "![Latest update](https://img.shields.io/badge/Latest%20build-$(Get-Date -Format 'MM/dd/yyyy')-blue?style=for-the-badge)"
...
function NewCONTRIBUTOR {
    ...
    # Define paths
    $gitFolder = ".git"
    $contribFile = ".github/CONTRIBUTOR.md"
    ...
    foreach ($line in $text -split "`n") {
        switch -Regex ($line) {
            ...
            "!\[itt\.xName:(.+?)\s*\[(.+?)\]\]\((.+?)\)" {
                # Image section
                $xaml += 
                "<Image x:Name=''$($matches[1].Trim())'' Cursor=''Hand'' ToolTip=''Click to visit'' Margin=''15,0,0,15'' Height=''Auto'' Width=''388'' HorizontalAlignment=''Left''>
                    <Image.Source>
